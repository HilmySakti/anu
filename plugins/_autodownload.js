...
handler.all = async function (m, { isPrems, isOwner }) {
    ...
    if (/^.*tiktok/i.test(m.text)) {
        ...
        //     await this.sendVideo(m.chat, json.nowm, '*SAGIRI BOT*', m, { thumbnail: buf })
        // }).catch(_ => _)
        let res = await fetch(global.API('hardianto', '/api/download/tiktok', { url: m.text.split` `[0] }, 'apikey'))
        if (!res.ok) throw await `${res.status} ${res.statusText}`
        let json = await res.json()
        await m.reply(global.wait)
        await conn.sendVideo(m.chat, json.wm, 'SAGIRI BOT', m)
    }
    ...
    if (/^.*cocofun/i.test(m.text)) {
        ...
        let json = await res.json()
        await m.reply(global.wait)
        await this.sendVideo(m.chat, json.download, `SAGIRI BOT`, m)
    }
    ...
    if (/^.*(fb.watch|facebook.com)/i.test(m.text)) {
        facebook(m.text.split` `[0]).then(async res => {
            ...
            await m.reply(global.wait)
            await this.sendVideo(m.chat, json.data[1] != undefined ? json.data[1].url : json.data[0].url, 'SAGIRI BOT', m)
        }).catch(_ => _)
    }
    ...
    if (/^.*instagram.com\/(p|reel|tv)/i.test(m.text)) {
        igdl(m.text.split` `[0]).then(async res => {
            ...
            for (let { downloadUrl, type } of json) {
                this.sendFile(m.chat, downloadUrl, 'ig' + (type == 'image' ? '.jpg' : '.mp4'), 'SAGIRI BOT', m, 0, buf)
            }
        }).catch(_ => _)
    }
    ...
    if (/^.*(pinterest.com\/pin|pin.it)/i.test(m.text)) {
        pin(m.text.split` `[0]).then(async res => {
            ...
            await m.reply(global.wait)
            await this.sendVideo(m.chat, json.data.url, `SAGIRI BOT`, m)
        }).catch(_ => _)
    }
    ...
    if (/^.*twitter.com\//i.test(m.text)) {
        twitter(m.text.split` `[0]).then(async res => {
            ...
            for (let { url } of json.data) {
                this.sendFile(m.chat, url, 'ig' + (/mp4/i.test(url) ? '.mp4' : '.jpg'), `SAGIRI BOT`, m, 0, buf)
            }
        }).catch(_ => _)
    }
...
*Ukuran File Video:* ${filesize}
          `.trim(),
            thumb, 'SAGIRI BOT', 'AUDIO', `.yta ${vid.url}`, 'VIDEO', `.yt ${vid.url}`)
    }
